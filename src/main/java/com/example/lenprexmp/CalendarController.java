...
public class CalendarController {
    ...
    private void loadAppointmentsForMonth() {
        appointments.clear(); // clear previous data
        LocalDate firstDay = currentYearMonth.atDay(1);
        LocalDate lastDay = currentYearMonth.atEndOfMonth();

        try {
            List<Appointment> allAppointments = DatabaseService.getAllInDateRange(Appointment.class,"appointment_date",firstDay,lastDay);

            for (Appointment app : allAppointments) {
                appointments.computeIfAbsent(app.getAppointment_date(), d -> new ArrayList<>()).add(app);
            }
        ...
        for (int row = 0; row < totalRows; row++) {
            for (int col = 0; col < 7; col++) {
                ...
                for (Appointment app : dayAppointments) {
                    ...
                    Label label = new Label(app.getTime() + " - " + app.getReason());
                    label.getStyleClass().add("appt-label");
                    label.setStyle("-fx-text-fill: " + getColorByType(app.getReason()) + ";");
    ...
    public void addAppointment(Appointment vstup) {
        List<Appointment> dayAppointments = appointments.computeIfAbsent(vstup.getAppointment_date(), d -> new ArrayList<>());

        // 🔒 Overenie: či už existuje stretnutie na rovnaký čas
        boolean alreadyExists = dayAppointments.stream()
                .anyMatch(app -> app.getTime().equals(vstup.getTime()));
        ...
        if (alreadyExists) {
            showAlert(
                    ...
                    bundle.getString("calendar.error.duplicate.content").replace("{time}", vstup.getTime())
            );
            return;
        }
        ...
        try {
            DatabaseController.saveOrUpdateAppointment(vstup);
        }
        catch (Exception e ){
        ...
        loadAppointmentsForMonth();
        updateCalendar();
        //dayAppointments.add(vstup);
        //updateCalendar();
    }


    public void removeAppointment(Appointment appointment) {
        List<Appointment> list = appointments.get(appointment.getAppointment_date());
        if (list != null) {
            list.removeIf(a -> a.getId().equals(appointment.getId()));
            try {
                DatabaseController.deleteAppointment(appointment.getId());
            ...
            if (list.isEmpty()) {
                ...
                appointments.remove(appointment.getAppointment_date());
            }
        }
        updateCalendar();
    }
...
// Vytvor popup okno
    ...
    private void showAppointmentDetailPopup(Appointment app, Labeled label) {
        try {
            ...
            Stage popup = new Stage();
            popup.setTitle(bundle.getString("appointment.detail.title") + " - " + app.getAppointment_date());
            popup.setScene(scene);
            popup.setResizable(false);
            popup.initOwner(calendarGrid.getScene().getWindow());
            popup.show();
